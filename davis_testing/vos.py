...
class VOSDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        for line in f:
            rows = line.split()
            jpgfile = self.datapath+rows[0]
            lblfile = self.savepath+rows[0]

            self.jpgfiles.append(jpgfile)
            self.lblfiles.append(lblfile)

        f.close()
        print(self.jpgfiles,self.lblfiles)
    ...
    def __getitem__(self, index):
        ...
        print(rsz_h, rsz_w)
        #rsz_h,rsz_w=rsz_h//2,rsz_w//2
        for i,p in enumerate(lbl_paths):
            prefix = p.replace('.png','')
            #print(prefix)
        ...
        print(resizes[0].shape,len(resizes))
