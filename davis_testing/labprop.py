...
dataset = VOSDataset(args)
val_loader = torch.utils.data.DataLoader(dataset,batch_size=1,
                                         shuffle=False, num_workers=0, pin_memory=True)

for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(val_loader):
    print(imgs.shape, imgs_orig.shape, lbls.shape, lbls_orig.shape, lbl_map.shape)
    break
    ...
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for b in range(0, imgs.shape[1], bsize):
                feat = model(imgs[:, b:b+bsize].squeeze().to(args.device))
                feats.append(feat.cpu())
            
            feats = torch.cat(feats, dim=0).unsqueeze(0).transpose(1,2)
            ...
            for t in range(key_indices.shape[0]):
                ...
                print(lbls[t + n_context].shape,pred.shape)
                if t > 0:
                ...
                print(cur_img.shape,lbl_map.shape,pred.shape)
